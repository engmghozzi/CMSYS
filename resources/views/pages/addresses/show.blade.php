                            ...
                            <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-200
                                ...
                                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                                    ...
                                    <div class="flex items-center gap-0.5">
                                        ...
                                        @endif
                                        @if(auth()->user()->hasPermission('contracts.update') && ($contract->status === 'active' && !$contract->is_expired || (auth()->user()->role && auth()->user()->role->name === 'super_admin' && ($contract->status === 'expired' || $contract->is_expired))))
                                        ...
                                        @elseif(auth()->user()->hasPermission('contracts.update') && ($contract->status !== 'active' || $contract->is_expired) && !(auth()->user()->role && auth()->user()->role->name === 'super_admin' && ($contract->status === 'expired' || $contract->is_expired)))
                                        ...
                                        @endif
                                        @if(auth()->user()->hasPermission('contracts.destroy') && ($contract->status === 'active' && !$contract->is_expired || (auth()->user()->role && auth()->user()->role->name === 'super_admin' && ($contract->status === 'expired' || $contract->is_expired))))
                                        ...
                                        @elseif(auth()->user()->hasPermission('contracts.destroy') && ($contract->status !== 'active' || $contract->is_expired) && !(auth()->user()->role && auth()->user()->role->name === 'super_admin' && ($contract->status === 'expired' || $contract->is_expired)))
                                            <span class="p-1.5 rounded-md bg-gray-50 text-gray-300 cursor-not-allowed" 
