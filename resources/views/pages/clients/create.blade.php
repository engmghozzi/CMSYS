<x-layouts.app :title="__('New Client')">
    <div class="min-h-screen">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">{{ __('Create New Client') }}</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200">
            <form method="POST" action="{{ route('clients.store') }}" enctype="multipart/form-data" class="p-4 sm:p-8 space-y-8">
                @csrf
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-3 pt-8 border-t border-gray-200">
                    <a href="{{ route('clients.index') }}" 
                        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors">
                        {{ __('Cancel') }}
                    </a>
                    <button type="submit" 
